...
if (form && grid && countEl) {
  ...
  function readURL(): { state: State; sort: Sort } {
    ...
    for (const axis of AXES) state[axis] = p.getAll(axis).filter(Boolean);
  ...
  function apply(state: State, sort: Sort): void {
    ...
    countEl!.textContent =
      visible === total ? `${total} recipes` : `${visible} of ${total} recipes`;
  ...
  // Hydrate from a deep-linked / shared URL on load.

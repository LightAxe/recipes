...
```
Parse `thread_id` from the `{"type":"thread.started","thread_id":"..."}` line → that's `THREAD_ID`. The critique is in `/tmp/codex-verdict.txt`. Confirm success by the verdict file + a `thread.started` line; if neither appears, the run failed (auth/model) — stop and tell the user. `2>/dev/null` suppresses cosmetic MCP/auth stderr noise. **`< /dev/null` is REQUIRED:** `codex exec` reads stdin *in addition to* the prompt arg, so under a non-TTY driver (Claude Code Bash tool, CI, any pipeline) it blocks forever at "Reading additional input from stdin..." (a silent ~0-CPU hang) without it.
